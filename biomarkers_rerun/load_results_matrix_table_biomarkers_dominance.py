...
# Note, for dominance, we don't run chrX.
def load_results(sex, pipeline, dilution):
    if dilution:
        # This is not created
        # ht_autosomes = hl.read_table(f'gs://ukbb-dominance/biomarkers/results/ukb31063.biomarker_gwas_results.{sex}.{contig}.pipeline_{pipeline}.dilution_factor_rerun.ht')
    else:
        ht_autosomes = hl.read_table(f'gs://ukbb-dominance/biomarkers/results/ukb31063.biomarker_gwas_results.{sex}.{contig}.pipeline_{pipeline}_rerun.ht')

    return ht_autosomes

# Note that pipelines 0-2 are not used because we do not run the raw phenotypes in the final analysis.
ht_results = load_results(sex, 3, dilution)
    ...
    ht_results = ht_results.annotate(
        entries=ht_results['entries'].extend(hl.rbind(
            ...
            lambda new_results: hl.array([
                hl.struct(
                    AC=new_results['sum_x'][i],
                    ytx=new_results['y_transpose_x'][i][0],
                    beta=new_results['beta'][i][0],
                    se=new_results['standard_error'][i][0],
                    tstat=new_results['t_stat'][i][0],
                    pval=new_results['p_value'][i][0])
...
if dilution:
    # This is not created
    # mt.write(f'gs://ukbb-dominance/biomarkers/results/combined/ukb31063.biomarker_gwas_results.{sex}.dilution_factor_rerun.mt', overwrite=True)
else:
    mt.write(f'gs://ukbb-dominance/biomarkers/results/combined/ukb31063.biomarker_gwas_results.{sex}_rerun.mt', overwrite=True)
