...
phenotypes_to_analyse = []
for phenotype in phenotypes:    
    if phenotype in ["apoliprotein_A_irnt", "igf_1_irnt", "shbg_irnt"]:
        phenotypes_to_analyse.append(phenotype)
...
if len(phenotypes_to_analyse) > 0:
    ...
    #     c_A = 1 / hl.sqrt(mt.a + mt.r - (mt.a - mt.r) ** 2),
    #     ha = mt.h * mt.a,
    #     ar = mt.a * mt.r,
    #     hr = mt.h * mt.r,
    #     h_2a = mt.h + 2 * mt.a,
    ...
    mt = mt.annotate_rows(
        c_D = 1 / hl.sqrt(( mt.a + mt.r - (mt.a - mt.r) ** 2)),
        ha = mt.h * mt.a,
        ar = mt.a * mt.r,
        hr = mt.h * mt.r,
    )
    ...
    if dilution:
        ...
        #     overwrite=True)
        ht.write(f'gs://ukbb-dominance/biomarkers/results/no_HWE_assumption/ukb31063.biomarker_gwas_results.{sex}.{contig}.pipeline_{pipeline}.dilution_factor_no_HWE_assumption_GP_without_var_rescale.ht',
            overwrite=True)
    else:
        ...
        #     overwrite=True)
        ht.write(f'gs://ukbb-dominance/biomarkers/results/no_HWE_assumption/ukb31063.biomarker_gwas_results.{sex}.{contig}.pipeline_{pipeline}_no_HWE_assumption_GP_without_var_rescale.ht',
            overwrite=True)
