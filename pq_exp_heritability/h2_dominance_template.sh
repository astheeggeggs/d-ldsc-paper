...
for alpha in {"-1.0","-0.5","-0.25","0.0","0.25","0.5","1.0"}; do
	...
	out="${path_to_out}/dom.alpha.${alpha}.${phenotype}" 
	...
	python /home/unix/dpalmer/Repositories/d-ldsc/get_h2.py --dominance --ref-ld-chr $ldscore_files --w-ld-chr $ldscore_files_weights --n-blocks 200 --write-h2 --h2 $sumstat_file --chisq-max 10000000000 --out $out --pheno-name $phenotype
done
